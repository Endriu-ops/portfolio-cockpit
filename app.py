...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v11.0 Stable", layout="wide")
...
SATELLITE_STATE_CSV = "satellite_state.csv"
MIN_SAT3_HOLDING_DAYS = 90
SAT3_EXIT_SCORE = 45.0
PANIC_VIX_LEVEL = 60.0

PRELOADED_PORTFOLIO = [{'ticker': 'IBGS.MI', 'shares': 98.0, 'avg_price': 139.9, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'MTHP.MI', 'shares': 193.0, 'avg_price': 73.1012, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'SGLD.MI', 'shares': 70.974, 'avg_price': 208.48, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'SXR8', 'shares': 10.419, 'avg_price': 588.85, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'ZPRV', 'shares': 407.0, 'avg_price': 66.241, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'BTC', 'shares': 0.126, 'avg_price': 70381.31, 'manual_price': 0, 'broker': '', 'note': 'BTC spot'}, {'ticker': 'CMOD.MI', 'shares': 460.0, 'avg_price': 27.195, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'GDX.MI', 'shares': 110.0, 'avg_price': 85.01, 'manual_price': 0, 'broker': '', 'note': ''}, {'ticker': 'RBOT.MI', 'shares': 0.0, 'avg_price': 0.0, 'manual_price': 0, 'broker': '', 'note': 'AI'}, {'ticker': 'SMH', 'shares': 46.0, 'avg_price': 101.18, 'manual_price': 0, 'broker': '', 'note': 'tattico 3%'}, {'ticker': 'VWCE', 'shares': 4.0, 'avg_price': 161.97, 'manual_price': 0, 'broker': '', 'note': 'PAC globale'}, {'ticker': 'VUAA', 'shares': 203.0, 'avg_price': 108.4737, 'manual_price': 0, 'broker': '', 'note': 'S&P500 equivalente'}]
...
ALPHA_STATIC = {"IBGS.MI":8, "MTHP.MI":4, "SGLD.MI":15, "VUSA.MI":22, "WSML.MI":19, "BTC":5, "CMOD.MI":8, "GDX.MI":6, "RBOT.MI":5, "XEON.MI":3}
...
def load_satellite_state():
    """
    Persistent SAT3 memory.
    Stores the currently active tactical ETF and its start date, so the
    90-day minimum holding rule survives Streamlit reruns and app restarts.
    """
    default = {"slot": "SAT3", "ticker": "", "theme": "", "macro": "", "start_date": "", "last_update": ""}
    if not os.path.exists(SATELLITE_STATE_CSV):
        return default
    try:
        df = pd.read_csv(SATELLITE_STATE_CSV)
        if df.empty:
            return default
        row = df.iloc[0].to_dict()
        return {**default, **{k: ("" if pd.isna(v) else str(v)) for k, v in row.items()}}
    except Exception:
        return default

def save_satellite_state(ticker, theme="", macro=""):
    state = pd.DataFrame([{
        "slot": "SAT3",
        "ticker": str(ticker or ""),
        "theme": str(theme or ""),
        "macro": str(macro or ""),
        "start_date": _utc_today_str(),
        "last_update": datetime.utcnow().isoformat(timespec="seconds")
    }])
    state.to_csv(SATELLITE_STATE_CSV, index=False)

def reset_satellite_state():
    if os.path.exists(SATELLITE_STATE_CSV):
        os.remove(SATELLITE_STATE_CSV)

def _sat3_days_held(state):
    try:
        start = pd.to_datetime(state.get("start_date", ""), errors="coerce")
        if pd.isna(start):
            return None
        return max(0, int((pd.Timestamp(datetime.utcnow().date()) - start.tz_localize(None).normalize()).days))
    except Exception:
        return None

def _ranking_row_for_ticker(rank_df, ticker):
    if rank_df is None or rank_df.empty or not ticker:
        return None
    m = rank_df[rank_df["Ticker"].astype(str) == str(ticker)]
    if m.empty:
        return None
    return m.iloc[0].to_dict()

def _sat3_cash_row(reason="Cash/Bond breve tactical"):
    return {"Slot":"SAT3","Theme":reason,"MacroSector":"Cash","Ticker":"XEON.MI","Score":None,"TargetWeight":3.0}

def satellite_plan(max_per_macro=1, vix_override=None):
    """
    v11.1 satellite:
    - SAT1 AI fixed 5%
    - SAT2 Defense fixed 5%
    - SAT3 tactical 3% with persistent 90-day minimum holding rule

    SAT3 may rotate before 90 days only when:
    - VIX panic / risk-off protocol is active
    - the currently held SAT3 score falls below SAT3_EXIT_SCORE
    """
    ...
    state = load_satellite_state()
    current_ticker = state.get("ticker", "")
    days_held = _sat3_days_held(state)
    days_to_unlock = None if days_held is None else max(0, MIN_SAT3_HOLDING_DAYS - days_held)

    risk_off = scs["RegimeScore"] < 45 or scs["DefensivePressure"] >= 80 or (v is not None and v >= PANIC_VIX_LEVEL)

    rank_all = ranking("5y", vix_override).dropna(subset=["Score"])
    current_rank_row = _ranking_row_for_ticker(rank_all, current_ticker)
    current_score = None
    if current_rank_row is not None and current_rank_row.get("Score") is not None:
        current_score = float(current_rank_row["Score"])
    current_exit_allowed = current_score is not None and current_score < SAT3_EXIT_SCORE

    # Candidate SAT3 from regime/ranking.
    if risk_off:
        candidate = _sat3_cash_row("Cash/Bond breve tactical — risk-off override")
    else:
        eligible = rank_all[rank_all["Score"] >= 75]
        excluded = {"AI_Automation", "Defense", "Technology"}
        chosen = None
        ...
        if chosen is None:
            candidate = _sat3_cash_row("Cash/Bond breve tactical — no score >= 75")
        else:
            candidate = {
                "Slot":"SAT3",
    ...
    cooldown_active = (
        bool(current_ticker)
        and candidate["Ticker"] != current_ticker
        and days_held is not None
        and days_held < MIN_SAT3_HOLDING_DAYS
        and not risk_off
        and not current_exit_allowed
    )

    if cooldown_active:
        # Keep the existing SAT3 until 90 days are completed.
        if current_rank_row is not None:
            sat3 = {
                "Slot":"SAT3",
                "Theme":str(current_rank_row["Theme"]) + " tactical — cooldown lock",
                "MacroSector":current_rank_row["MacroSector"],
                "Ticker":current_ticker,
                "Score":current_rank_row["Score"],
                "TargetWeight":3.0
            }
        else:
            sat3 = {
                "Slot":"SAT3",
                "Theme":(state.get("theme") or "Current SAT3") + " — cooldown lock",
                "MacroSector":state.get("macro") or "Tactical",
                "Ticker":current_ticker,
                "Score":current_score,
                "TargetWeight":3.0
            }
        sat3["CooldownStatus"] = "LOCKED"
        sat3["DaysHeld"] = days_held
        sat3["DaysToUnlock"] = days_to_unlock
        sat3["RecommendedTicker"] = candidate["Ticker"]
    else:
        sat3 = candidate
        sat3["CooldownStatus"] = "OPEN" if not risk_off else "RISK_OFF_OVERRIDE"
        sat3["DaysHeld"] = 0 if candidate["Ticker"] != current_ticker else days_held
        sat3["DaysToUnlock"] = MIN_SAT3_HOLDING_DAYS if candidate["Ticker"] != current_ticker else days_to_unlock
        sat3["RecommendedTicker"] = candidate["Ticker"]

        if candidate["Ticker"] != current_ticker:
            save_satellite_state(candidate["Ticker"], candidate.get("Theme",""), candidate.get("MacroSector",""))
...
st.title("Portfolio Cockpit Alpha Pro v11.0 Stable")
st.caption("Decision Engine v11.0 Stable: clean stable build, DFNS ticker, safe modules, aligned tabs.")
...
# ======================================================
# V10.6.0 STABLE FALLBACKS
# ======================================================

def geographic_pac_priority_safe(rt, pac_amount, regime_payload=None):
    try:
        return geographic_pac_priority(rt, pac_amount, regime_payload, 0)
    except Exception:
        pass
    if rt is None or rt.empty or pac_amount <= 0:
        return pd.DataFrame()
    if "market_value" not in rt.columns or rt["market_value"].sum() <= 0:
        return pd.DataFrame()
    total = float(rt["market_value"].sum())
    rows = []
    held = rt[pd.to_numeric(rt.get("shares", 0), errors="coerce").fillna(0) > 0].copy()
    vwce = held[held["ticker"].astype(str).isin(["VWCE", "VWCE.MI"])]
    vwce_w = float(vwce["market_value"].sum() / total * 100) if not vwce.empty else 0.0
    if vwce_w > 0 and vwce_w < 10:
        rows.append({
            "priority": 1,
            "ticker": "VWCE",
            "action": "PAC BUY",
            "reason": "Safe fallback: aumento diversificazione geografica VWCE",
            "allocation_eur": pac_amount * 0.50,
            "execution_status": "EXECUTABLE_EXISTING_HOLDING",
            "engine": "V10.6.0 Safe Fallback"})
    remaining = pac_amount - sum(r["allocation_eur"] for r in rows)
    if remaining > 0 and "deviation_pp" in held.columns:
        under = held[(held["deviation_pp"] < 0) & (~held["ticker"].astype(str).isin(["VWCE", "VWCE.MI"]))].copy()
        if not under.empty:
            under = under.sort_values("deviation_pp").head(2)
            gaps = under["deviation_pp"].abs()
            if gaps.sum() <= 0:
                gaps = pd.Series([1] * len(under), index=under.index)
            for _, r in under.iterrows():
                share = abs(float(r["deviation_pp"])) / gaps.sum()
                rows.append({
                    "priority": 2,
                    "ticker": r["ticker"],
                    "action": "PAC BUY",
                    "reason": "Safe fallback: existing holdings first",
                    "allocation_eur": remaining * share,
                    "execution_status": "EXECUTABLE_EXISTING_HOLDING",
                    "engine": "V10.6.0 Safe Fallback"})
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    s = out["allocation_eur"].sum()
    if s > 0:
        out["allocation_eur"] = out["allocation_eur"] / s * pac_amount
    return out.sort_values(["priority", "allocation_eur"], ascending=[True, False])

def eem_watchlist_policy_safe(rt):
    try:
        return eem_watchlist_policy(rt, 0)
    except Exception:
        return pd.DataFrame([{
            "Ticker": "EEM",
            "PolicyStatus": "WATCHLIST_ONLY",
            "Reason": "Nuovi ETF bloccati di default; EEM solo watchlist."
        }])



# ======================================================
# V10.6.0 STABLE GUARDS
# ======================================================
def geographic_pac_priority(*args, **kwargs):
    return pd.DataFrame()
def geographic_pac_priority_safe(*args, **kwargs):
    return pd.DataFrame()
def eem_watchlist_policy(*args, **kwargs):
    return pd.DataFrame()
def eem_watchlist_policy_safe(*args, **kwargs):
    return pd.DataFrame()
def vwce_path_simulation(*args, **kwargs):
    return pd.DataFrame(columns=["Month","VWCEWeightPct","PortfolioValueApprox"])
# ======================================================
...
# V11.0 STABLE GUARDS
# ======================================================
def _stable_empty_df(*args, **kwargs):
    return pd.DataFrame()
...
def safe_call_df(fn, *args, **kwargs):
    try:
...
with tabs[2]:  # Geographic Engine
    ...
    if rt is None or rt.empty:
        ...
        regime_payload_geo = regime_engine_v90(vix_override) if "regime_engine_v90" in globals() else {"Regime":"NEUTRAL"}
        ...
        st.write("### PAC Priority Order")
        st.info("Modulo sperimentale Geographic PAC disattivato in V11 Stable. Usa PAC Advisor operativo.")
        st.write("### EEM / nuovi ETF policy")
        st.info("Nuovi ETF bloccati di default; EEM resta solo watchlist manuale.")
        st.write("### Simulazione crescita VWCE")
        st.info("Simulazione VWCE disattivata in V11 Stable.")
with tabs[3]:
...
with tabs[10]:
    st.subheader("Satellite Auto — SAT3 cooldown 90 giorni")
    st.caption("SAT3 può ruotare solo dopo 90 giorni, salvo risk-off/VIX panic o score dell'attuale SAT3 sotto 45.")
    sat_state = load_satellite_state()
    plan=satellite_plan(vix_override=vix_override)
    sat3_row = plan[plan["Slot"]=="SAT3"].iloc[0] if not plan[plan["Slot"]=="SAT3"].empty else None
    c_sat1, c_sat2, c_sat3 = st.columns(3)
    c_sat1.metric("SAT3 attuale", sat3_row["Ticker"] if sat3_row is not None else "N/A")
    c_sat2.metric("Giorni detenuto", sat3_row.get("DaysHeld", "N/A") if sat3_row is not None else "N/A")
    c_sat3.metric("Cooldown", sat3_row.get("CooldownStatus", "N/A") if sat3_row is not None else "N/A")
    st.dataframe(plan,use_container_width=True)
    st.caption(f"Stato persistente: {sat_state}")
    if st.button("Reset stato SAT3 / nuova decorrenza"):
        reset_satellite_state()
        st.warning("Stato SAT3 resettato. Alla prossima esecuzione verrà salvato il SAT3 scelto dal ranking corrente.")
        st.rerun()
with tabs[11]:
...
with tabs[18]:
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v11.0 Stable: test alert.")
