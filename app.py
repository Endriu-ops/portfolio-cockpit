...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v8.0", layout="wide")
...
def pac_advisor(real_df, pac_amount, tax_rate=26.0, vix=None):
    ...
    for _, r in underweight.iterrows():
        weight_share = r["deviation_pp"].abs() / total_under if total_under > 0 else 0
...
# UI
# ══════════════════════════════════════════════════════════════════════════════
st.title("Portfolio Cockpit Alpha Pro v8.0")
st.caption("v8.0 — Beta stress test · PAC intelligente · Ordini prioritizzati · LEI/PCE/P/C · Sanity check prezzi · UX semplificata")
...
with tabs[1]:
    ...
    if csave2.button("Carica CSV salvato"):
        st.session_state["portfolio_df"]=load_portfolio_csv(vix_override); st.rerun()
...
with tabs[2]:
    ...
    if "real_table" not in st.session_state:
        ...
        orders = generate_order_list(
            st.session_state["real_table"], tax_rate, tax_budget,
...
with tabs[3]:
    ...
    if "real_table" not in st.session_state:
        ...
        st.info(f"Rata mensile configurata: **€{pac_amount:,.0f}** | VIX: {current_vix:.1f if current_vix else 'n/d'}")
        pac_df = pac_advisor(st.session_state["real_table"], pac_amount, tax_rate, current_vix)
        if not pac_df.empty:
...
with tabs[22]:  # Alerts
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v8.0: test alert.")
