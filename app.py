...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v3", layout="wide")
...
GOLDEN_BUTTERFLY = {
    "IBGS.MI":20, "MTHP.MI":20, "SGLD.MI":20, "VUSA.MI":20, "WSML.MI":20
}

ALPHA_STATIC = {
    "IBGS.MI":8, "MTHP.MI":4, "SGLD.MI":15, "VUSA.MI":22, "WSML.MI":19,
    "CBTC.MI":5, "CMOD.MI":8, "GDX.MI":6, "DFEN.MI":5, "RBOT.MI":5, "XEON.MI":3
}
...
def liquidity_score():
    ...
    if not m2.empty and len(m2)>13:
        parts.append(bscore(m2["value"].iloc[-1]/m2["value"].iloc[-13]-1,20))
    ...
    if not nfci.empty:
        parts.append(max(0,min(100,60-nfci["value"].iloc[-1]*40)))
    return mean(parts)
...
def macro_score():
    ...
    if not cf.empty:
        parts.append(max(0,min(100,50+cf["value"].iloc[-1]*25)))
    if not un.empty and len(un)>6:
        parts.append(max(0,min(100,55-(un["value"].iloc[-1]-un["value"].iloc[-7])*35)))
    if not ip.empty and len(ip)>13:
        parts.append(bscore(ip["value"].iloc[-1]/ip["value"].iloc[-13]-1,30))
    if not retail.empty and len(retail)>13:
        parts.append(bscore(retail["value"].iloc[-1]/retail["value"].iloc[-13]-1,15))
    return mean(parts)

def market_score():
    parts=[]; v=last("^VIX")
    if v is not None:
        parts.append(90 if v<15 else 75 if v<20 else 55 if v<25 else 35 if v<30 else 20)
    for t in ["^GSPC","^NDX","^RUT"]:
        df=hist(t,"2y")
        if not df.empty:
            parts.append(trend(df["Close"]))
    return mean(parts)
...
def tact_score(ticker, scs):
    ...
    if pd.isna(mo) or pd.isna(tr):
        return np.nan
    if pd.isna(rs):
        rs=50
    vv=vol(c,63)
    penalty=max(0,min(30,(vv-.25)*70)) if not pd.isna(vv) else 0
...
def ranking(period="5y"):
    ...
    for theme,meta in TACTICAL.items():
        ...
        for t in meta["tickers"]:
            ...
            row={
                "Theme":theme,"MacroSector":meta["macro"],"Ticker":t,
...
def satellite_plan(max_per_macro=1):
    df=ranking("5y").dropna(subset=["Score"])
    df=df[df["Score"]>=75]
    selected=[]; used_macro={}
    for _,r in df.iterrows():
        macro=r["MacroSector"]
        if used_macro.get(macro,0) >= max_per_macro:
            continue
        selected.append(r)
        used_macro[macro]=used_macro.get(macro,0)+1
        if len(selected)==3:
            break
    out=[]
...
st.title("Portfolio Cockpit Alpha Pro v3")
st.caption("Cycle-aware satellite + anti-concentration + enhanced backtest + tax-aware rebalance.")
...
tabs=st.tabs([
    "Allocation","My Portfolio","Scores","Market","FRED","Relative Strength",
    "Tactical Ranking","Satellite Auto","Backtest","Tax-Aware Rebalance","Paper Trading","Alerts"
])
...
with tabs[1]:
    ...
    for _,r in plan.iterrows():
        rows.append({"ticker":r["Ticker"],"shares":0.0,"avg_price":0.0})
...
with tabs[3]:
    ...
    for name,t in MARKET.items():
        df=hist(t,period)
        if df.empty:
            out.append({"Indicator":name,"Ticker":t,"Last":None})
            continue
...
with tabs[4]:
    ...
    for name,sid in FRED.items():
        df=fred(sid)
        if df.empty:
            out.append({"Indicator":name,"Series":sid,"Last":None,"Date":None})
        else:
            v=df["value"]
...
with tabs[5]:
    ...
    for name,(a,b) in RS_PAIRS.items():
        da,db=hist(a,"2y"),hist(b,"2y")
        if da.empty or db.empty:
            out.append({"Pair":name,"A":a,"B":b,"Score":None})
            continue
        r=(da["Close"]/db["Close"]).dropna()
...
with tabs[6]:
    ...
    if not top.empty:
        st.plotly_chart(px.bar(top,x="Theme",y="Score",color="MacroSector"),use_container_width=True)

with tabs[7]:
    plan=satellite_plan(max_per_macro=1)
    st.dataframe(plan,use_container_width=True)
    msg="Satellite automatico v3:\n"+"\n".join([f"{r['Slot']}: {r['Ticker']} {r['TargetWeight']}% ({r['Theme']})" for _,r in plan.iterrows()])
    st.code(msg)
    st.caption("Filtro anti-concentrazione: massimo 1 tema per macro-settore nel satellite.")
    if st.button("Invia piano via Telegram"):
        ok,resp=send_telegram(msg); st.success("Inviato") if ok else st.error(resp)

with tabs[8]:
    st.subheader("Backtest comparativo migliorato")
    st.write("Ribilanciamento mensile/annuale semplificato. Non include tasse, spread, costi e tracking error.")
    ...
    if st.button("Esegui backtest comparativo"):
        dyn=dynamic_alpha_weights()
        series={}
        gb=backtest(normalize(GOLDEN_BUTTERFLY),btperiod,months)
        ap=backtest(normalize(ALPHA_STATIC),btperiod,months)
        ad=backtest(normalize(dyn),btperiod,months)
        spy=backtest({"SPY":1.0},btperiod,months)
        if not gb.empty: series["Golden Butterfly proxy"]=gb
        if not ap.empty: series["Alpha Pro static"]=ap
        if not ad.empty: series["Alpha Pro dynamic current"]=ad
...
with tabs[9]:
    st.subheader("Tax-Aware Rebalance indicativo")
    if "real_table" not in st.session_state:
        st.warning("Prima aggiorna la tab My Portfolio.")
    else:
        tab=st.session_state["real_table"].copy()
        sells=tab[tab["trade_value"]<0].copy()
        if sells.empty:
            st.success("Nessuna vendita richiesta dal rebalance.")
        else:
            ...
            st.dataframe(sells[["ticker","market_value","pnl","pnl_pct","sell_amount","taxable_gain_est","tax_est"]],use_container_width=True)
            st.metric("Tasse stimate su vendite", f"{sells['tax_est'].sum():,.2f}")
...
with tabs[11]:
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v3: test alert.")
