# ═══════════════════════════════════════════════════════════════════════════════
# Portfolio Cockpit Alpha Pro — v8.3
...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v10.0", layout="wide")
...
# ======================================================
# V10.0 INSTITUTIONAL LAYER
# ======================================================

DECISION_LOG_CSV = "decision_log.csv"
TRADE_BLOTTER_CSV = "trade_blotter.csv"

def institutional_score_grade(score):
    try:
        s = float(score)
    except Exception:
        return "N/A"
...
def portfolio_health_pro(real_df, regime_payload=None, tax_status=None):
    if real_df is None or real_df.empty or "market_value" not in real_df.columns or real_df["market_value"].sum() <= 0:
        return {"TotalScore": 0, "Grade": "N/A", "Concentration": 0, "Diversification": 0, "DataQuality": 0, "Drift": 0, "Tax": 0, "RegimeFit": 0}
    df = real_df.copy()
    total = df["market_value"].sum()
    weights = df["market_value"] / total
    max_w = float(weights.max() * 100)
    n_eff = float(1 / (weights**2).sum()) if (weights**2).sum() > 0 else 0
    concentration_score = max(0, min(100, 100 - max(0, max_w - 15) * 3))
    diversification_score = max(0, min(100, n_eff / 10 * 100))
    data_quality = 100
    if "price_status" in df.columns:
        data_quality -= int(df["price_status"].isin(["MISSING"]).sum()) * 25
        data_quality -= int(df["price_status"].isin(["AVG_FALLBACK"]).sum()) * 10
        data_quality = max(0, data_quality)
    if "deviation_pp" in df.columns:
        avg_abs_drift = float(df["deviation_pp"].abs().mean())
        max_abs_drift = float(df["deviation_pp"].abs().max())
        drift_score = max(0, min(100, 100 - avg_abs_drift * 8 - max(0, max_abs_drift - 10) * 3))
    else:
        drift_score = 70
    tax_score = 75
    if tax_status:
        remaining = tax_status.get("remaining_tax_budget", 0)
        budget = tax_status.get("tax_budget", 1)
        tax_score = max(0, min(100, remaining / budget * 100)) if budget > 0 else 75
    regime_fit = 75
    if regime_payload:
        regime = regime_payload.get("Regime", "NEUTRAL")
        sat_tickers = SATELLITE_TICKERS_V82 if "SATELLITE_TICKERS_V82" in globals() else ["RBOT.MI","DFNS.MI","SMH"]
        satellite_weight = float(df[df["ticker"].astype(str).isin(sat_tickers)]["market_value"].sum() / total * 100)
        if regime == "RISK ON": regime_fit = 100 if satellite_weight >= 10 else 75
        elif regime == "NEUTRAL": regime_fit = 100 if 7 <= satellite_weight <= 16 else 70
        elif regime == "RISK OFF": regime_fit = 100 if satellite_weight <= 10 else 50
        elif regime == "CRISIS": regime_fit = 100 if satellite_weight <= 5 else 35
    total_score = 0.20*concentration_score + 0.20*diversification_score + 0.20*data_quality + 0.15*drift_score + 0.15*tax_score + 0.10*regime_fit
    return {"TotalScore": round(total_score,1), "Grade": institutional_score_grade(total_score), "Concentration": round(concentration_score,1), "Diversification": round(diversification_score,1), "DataQuality": round(data_quality,1), "Drift": round(drift_score,1), "Tax": round(tax_score,1), "RegimeFit": round(regime_fit,1), "MaxWeightPct": round(max_w,2), "EffectivePositions": round(n_eff,2)}

def alpha_score_pro(real_df=None, vix_override=None):
    if "alpha_score_table_v82" in globals():
        df = alpha_score_table_v82(real_df, vix_override)
    else:
        df = compute_alpha_scores(real_df).rename(columns={"ticker":"Ticker"}) if real_df is not None else pd.DataFrame()
    if df is None or df.empty:
        return pd.DataFrame()
    df = df.copy()
    if "AlphaScore" not in df.columns: df["AlphaScore"] = 50
    if "AssetGroup" not in df.columns:
        df["AssetGroup"] = df["Ticker"].apply(asset_group_v82) if "asset_group_v82" in globals() else "Other"
    regime = regime_engine_v90(vix_override) if "regime_engine_v90" in globals() else {"Regime":"NEUTRAL","Score":50}
    reg = regime.get("Regime","NEUTRAL")
    mult = {"RISK ON":1.10, "NEUTRAL":1.00, "RISK OFF":0.75, "CRISIS":0.45}.get(reg, 1.0)
    df["Regime"] = reg
    df["RegimeMultiplier"] = mult
    df["InstitutionalAlpha"] = (df["AlphaScore"].astype(float) * mult).clip(0,100)
    df["AlphaGrade"] = df["InstitutionalAlpha"].apply(institutional_score_grade)
    df["InstitutionalAction"] = np.where(df["InstitutionalAlpha"]>=80, "ACCUMULATE", np.where(df["InstitutionalAlpha"]>=65, "HOLD / SELECTIVE BUY", np.where(df["InstitutionalAlpha"]>=50, "WATCHLIST", "AVOID / REDUCE")))
    return df.sort_values("InstitutionalAlpha", ascending=False)

def risk_budget_pro(real_df=None, vix_override=None):
    regime = regime_engine_v90(vix_override) if "regime_engine_v90" in globals() else {"Regime":"NEUTRAL","Score":50,"VIX":None}
    budget = regime_risk_budget_v90(regime["Regime"]) if "regime_risk_budget_v90" in globals() else {"SatelliteTarget":13,"TacticalTarget":3,"CashTarget":0,"PACMode":"Alpha","RiskBudget":"NORMAL"}
    current_sat = 0.0
    current_core = 0.0
    if real_df is not None and not real_df.empty and "market_value" in real_df.columns and real_df["market_value"].sum() > 0:
        total = real_df["market_value"].sum()
        sat_tickers = SATELLITE_TICKERS_V82 if "SATELLITE_TICKERS_V82" in globals() else ["RBOT.MI","DFNS.MI","SMH"]
        current_sat = real_df[real_df["ticker"].astype(str).isin(sat_tickers)]["market_value"].sum()/total*100
        current_core = 100-current_sat
    return pd.DataFrame([{"Regime":regime["Regime"],"RegimeScore":regime["Score"],"VIX":regime.get("VIX"),"RiskBudget":budget["RiskBudget"],"CurrentSatellitePct":current_sat,"TargetSatellitePct":budget["SatelliteTarget"],"SatelliteGapPct":budget["SatelliteTarget"]-current_sat,"CurrentCorePct":current_core,"CashTargetPct":budget["CashTarget"],"PACMode":budget["PACMode"]}])

def investment_committee_memo(real_df=None, vix_override=None, tax_rate=26.0, tax_budget=2000.0, pac_amount=500.0):
    regime = regime_engine_v90(vix_override) if "regime_engine_v90" in globals() else {"Regime":"NEUTRAL","Score":50,"VIX":None,"CrisisFlags":"None"}
    budget = regime_risk_budget_v90(regime["Regime"]) if "regime_risk_budget_v90" in globals() else {"RiskBudget":"NORMAL","PACMode":"Alpha"}
    tax_status = tax_budget_status(tax_budget, tax_rate) if "tax_budget_status" in globals() else {"remaining_tax_budget":tax_budget,"tax_budget":tax_budget}
    health = portfolio_health_pro(real_df, regime, tax_status)
    recommendation = {"RISK ON":"ALLOW ALPHA DEPLOYMENT","NEUTRAL":"BALANCED / SELECTIVE","RISK OFF":"DEFENSIVE REBALANCE","CRISIS":"CRISIS PROTOCOL"}.get(regime["Regime"],"HOLD")
    rationale = []
    if regime["Regime"] == "RISK ON": rationale.append("Regime favorevole: satellite e PAC Alpha consentiti.")
    elif regime["Regime"] == "NEUTRAL": rationale.append("Regime neutrale: mantenere satellite standard e selettività.")
    elif regime["Regime"] == "RISK OFF": rationale.append("Regime risk-off: ridurre nuovi acquisti satellite e usare PAC sul core.")
    else: rationale.append("Regime crisis: congelare satellite e usare acquisti a tranche su core difensivo.")
    if health["TotalScore"] < 65: rationale.append("Portfolio Health Pro sotto soglia istituzionale: priorità a qualità dati/drift/rischio.")
    if tax_status.get("remaining_tax_budget",0) <= 0: rationale.append("Tax budget esaurito: vendite tassabili da differire.")
    return {"Date":datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),"Regime":regime["Regime"],"RegimeScore":round(regime["Score"],1),"VIX":regime.get("VIX"),"RiskBudget":budget.get("RiskBudget"),"PACMode":budget.get("PACMode"),"PortfolioHealthPro":health["TotalScore"],"Grade":health["Grade"],"TaxBudgetRemaining":tax_status.get("remaining_tax_budget"),"CrisisFlags":regime.get("CrisisFlags","None"),"Recommendation":recommendation,"Rationale":rationale}

def memo_to_markdown(memo):
    lines = ["# Investment Committee Memo", f"**Date:** {memo.get('Date')}", f"**Regime:** {memo.get('Regime')} ({memo.get('RegimeScore')}/100)", f"**VIX:** {memo.get('VIX')}", f"**Risk Budget:** {memo.get('RiskBudget')}", f"**PAC Mode:** {memo.get('PACMode')}", f"**Recommendation:** {memo.get('Recommendation')}", f"**Portfolio Health Pro:** {memo.get('PortfolioHealthPro')} — Grade {memo.get('Grade')}", f"**Tax Budget Remaining:** €{memo.get('TaxBudgetRemaining'):,.2f}" if memo.get("TaxBudgetRemaining") is not None else "**Tax Budget Remaining:** n/a", f"**Crisis Flags:** {memo.get('CrisisFlags')}", "", "## Rationale"]
    for r in memo.get("Rationale", []): lines.append(f"- {r}")
    return "\n".join(lines)

def load_decision_log():
    cols=["date","regime","recommendation","portfolio_health","tax_budget_remaining","note"]
    return load_csv_safe(DECISION_LOG_CSV, cols) if "load_csv_safe" in globals() else pd.DataFrame(columns=cols)

def save_decision_log_entry(memo, note=""):
    log=load_decision_log()
    row=pd.DataFrame([{"date":memo.get("Date"),"regime":memo.get("Regime"),"recommendation":memo.get("Recommendation"),"portfolio_health":memo.get("PortfolioHealthPro"),"tax_budget_remaining":memo.get("TaxBudgetRemaining"),"note":note}])
    out=pd.concat([log,row],ignore_index=True)
    if "save_csv_safe" in globals(): save_csv_safe(out, DECISION_LOG_CSV)
    else: out.to_csv(DECISION_LOG_CSV,index=False)
    return out

def trade_blotter_institutional(real_df=None, vix_override=None, tax_rate=26.0, tax_budget=2000.0, pac_amount=500.0):
    if real_df is None or real_df.empty: return pd.DataFrame()
    regime = regime_engine_v90(vix_override) if "regime_engine_v90" in globals() else {"Regime":"NEUTRAL","VIX":None}
    try:
        orders = generate_order_list(real_df, tax_rate, tax_budget, False, regime.get("VIX"), pac_amount)
    except Exception:
        orders = pd.DataFrame()
    if orders is None or orders.empty: return orders
    if "regime_trade_guard_v90" in globals(): orders = regime_trade_guard_v90(orders, regime)
    if "regime_guard" not in orders.columns: orders["regime_guard"]="OK"
    if "tax_drag_pct" not in orders.columns: orders["tax_drag_pct"]=0
    orders["institutional_status"] = np.where(orders["regime_guard"].astype(str).eq("BLOCK_NEW_SATELLITE_BUY"),"BLOCKED_BY_REGIME",np.where(orders["tax_drag_pct"].astype(float)>10,"TAX_REVIEW","APPROVED_REVIEW"))
    orders["ic_note"] = np.where(orders["institutional_status"].eq("BLOCKED_BY_REGIME"),"Regime guard: no new satellite buy in Risk Off/Crisis.",np.where(orders["institutional_status"].eq("TAX_REVIEW"),"Tax drag above threshold.","Action allowed subject to execution check."))
    return orders


# ======================================================
# V10.0 REGIME & CRISIS ENGINE
# ======================================================

def safe_last_close(ticker, period="1y"):
    df = hist(ticker, period)
...
def trend_200dma_signal(ticker):
    df = hist(ticker, "2y")
    if df is None or df.empty or "Close" not in df.columns or len(df["Close"].dropna()) < 220:
        return {"ticker": ticker, "above_200dma": None, "distance_200dma_pct": None, "score": 50}
    c = df["Close"].dropna()
    last_px = float(c.iloc[-1])
    ma200 = float(c.rolling(200).mean().iloc[-1])
    dist = last_px / ma200 - 1 if ma200 > 0 else 0
    score = 80 if dist > 0.05 else 65 if dist > 0 else 40 if dist > -0.05 else 20
    return {"ticker": ticker, "above_200dma": last_px > ma200, "distance_200dma_pct": dist * 100, "score": score}

def vix_regime_score(vix):
...
def dollar_risk_score():
    d = hist("DX-Y.NYB", "1y")
    if d is None or d.empty or "Close" not in d.columns or len(d["Close"].dropna()) < 130:
        return 50
    c = d["Close"].dropna()
    mom6 = pc(c, 126)
    # Dollar rising strongly is usually liquidity tightening / risk headwind.
    if pd.isna(mom6): return 50
    return max(0, min(100, 55 - mom6 * 250))

def rates_risk_score():
    tnx = hist("^TNX", "1y")
    if tnx is None or tnx.empty or "Close" not in tnx.columns or len(tnx["Close"].dropna()) < 65:
        return 50
    c = tnx["Close"].dropna()
    chg3m = c.iloc[-1] - c.iloc[-64] if len(c) > 64 else 0
    # Falling/stable rates support duration and risk; fast rising rates penalize.
    if chg3m < -0.4: return 75
    if chg3m < 0.2: return 60
    if chg3m < 0.7: return 40
    return 20

def breadth_regime_score():
    parts = []
    for a,b in [("RSP","SPY"), ("IWM","SPY"), ("HYG","IEF"), ("QQQ","XLU")]:
        da, db = hist(a, "2y"), hist(b, "2y")
        if da is not None and db is not None and not da.empty and not db.empty and "Close" in da.columns and "Close" in db.columns:
            r = (da["Close"] / db["Close"]).dropna()
            if len(r) > 126:
                parts.append(bscore(pc(r,126), 15))
    return mean(parts, 50)

def regime_engine_v90(vix_override=None):
    current_vix = vix_override if vix_override is not None else last("^VIX")

    sp = trend_200dma_signal("SPY")
    qqq = trend_200dma_signal("QQQ")
    iwm = trend_200dma_signal("IWM")

    vix_s = vix_regime_score(current_vix)
    trend_s = mean([sp["score"], qqq["score"], iwm["score"]], 50)
    breadth_s = breadth_regime_score()
    credit_s = credit_score()
    macro_s = macro_score()
    rates_s = rates_risk_score()
    dollar_s = dollar_risk_score()
    liquidity_s = liquidity_score()
    ...
    # Hard crisis overrides
    crisis_flags = []
    if current_vix is not None and current_vix >= 50:
        crisis_flags.append("VIX>=50")
    if sp["above_200dma"] is False and qqq["above_200dma"] is False and vix_s < 40:
        crisis_flags.append("SPY+QQQ sotto 200DMA")
    if credit_s < 30:
        crisis_flags.append("Credit stress")

    if current_vix is not None and current_vix >= 70:
        regime = "CRISIS"
    elif crisis_flags and composite < 45:
        regime = "RISK OFF"
    elif composite >= 68:
        regime = "RISK ON"
    elif composite >= 50:
    ...
    components = pd.DataFrame([
        {"Component": "VIX", "Score": vix_s, "Weight": 18, "Note": current_vix},
        {"Component": "Trend 200DMA", "Score": trend_s, "Weight": 18, "Note": f"SPY {sp['distance_200dma_pct']}, QQQ {qqq['distance_200dma_pct']}"},
        {"Component": "Breadth", "Score": breadth_s, "Weight": 14, "Note": "RSP/SPY, IWM/SPY, HYG/IEF, QQQ/XLU"},
        {"Component": "Credit", "Score": credit_s, "Weight": 14, "Note": "HY spreads + HYG/IEF"},
        {"Component": "Macro", "Score": macro_s, "Weight": 12, "Note": "LEI, claims, CFNAI, curve"},
        {"Component": "Liquidity", "Score": liquidity_s, "Weight": 10, "Note": "M2, real rates, NFCI"},
        {"Component": "Rates", "Score": rates_s, "Weight": 7, "Note": "US10Y 3M change"},
        {"Component": "Dollar", "Score": dollar_s, "Weight": 7, "Note": "DXY momentum"},
    ])
    ...
    return {
        ...
        "VIX": current_vix,
        "CrisisFlags": ", ".join(crisis_flags) if crisis_flags else "None",
...
def regime_risk_budget_v90(regime):
    if regime == "RISK ON":
        return {"SatelliteTarget": 15.0, "TacticalTarget": 5.0, "CashTarget": 0.0, "PACMode": "Alpha", "RiskBudget": "FULL"}
    if regime == "NEUTRAL":
        return {"SatelliteTarget": 13.0, "TacticalTarget": 3.0, "CashTarget": 2.0, "PACMode": "Alpha", "RiskBudget": "NORMAL"}
    if regime == "RISK OFF":
        return {"SatelliteTarget": 7.0, "TacticalTarget": 0.0, "CashTarget": 6.0, "PACMode": "Rebalance", "RiskBudget": "DEFENSIVE"}
    return {"SatelliteTarget": 0.0, "TacticalTarget": 0.0, "CashTarget": 13.0, "PACMode": "Rebalance", "RiskBudget": "CRISIS"}

def crisis_protocol_v90(regime_payload):
    regime = regime_payload.get("Regime", "NEUTRAL")
    vix = regime_payload.get("VIX", None)
    flags = regime_payload.get("CrisisFlags", "")

    if regime == "RISK ON":
        return [
            "Satellite attivo: AI/Defense/Tactical consentiti.",
            "PAC Alpha consentito.",
            "Lascia correre i vincitori entro bande dinamiche.",
            "Vendite solo se fiscalmente efficienti."
        ]
    if regime == "NEUTRAL":
        return [
            "Satellite standard 13%.",
            "PAC Alpha moderato.",
            "Nessuna rotazione aggressiva.",
            "Ribilanciamento solo oltre banda dinamica."
        ]
    if regime == "RISK OFF":
        return [
            "Riduci nuove esposizioni satellite.",
            "PAC verso core sottopesato, oro/bond/cash.",
            "Blocca Tactical se sotto trend.",
            "Evita vendite in panico; usa tax optimizer."
        ]
    return [
        "CRISIS MODE: congela nuovi acquisti satellite.",
        "PAC solo a tranche su core diversificato.",
        "Nessuna vendita emotiva.",
        "Deploy cash progressivo se VIX >70/>80.",
        f"Flags: {flags}; VIX: {vix}"
    ]

def regime_adjusted_satellite_plan_v90(vix_override=None, mode="Static"):
    payload = regime_engine_v90(vix_override)
    budget = regime_risk_budget_v90(payload["Regime"])

    if payload["Regime"] in ["RISK ON", "NEUTRAL"]:
        base = dynamic_satellite_plan_v82(vix_override=vix_override, mode=mode) if "dynamic_satellite_plan_v82" in globals() else satellite_plan(vix_override=vix_override)
        if base is None or base.empty:
            return base
        out = base.copy()
        scale = budget["SatelliteTarget"] / max(0.01, out["TargetWeight"].sum())
        out["TargetWeight"] = out["TargetWeight"] * scale
        out["Regime"] = payload["Regime"]
        out["RiskBudget"] = budget["RiskBudget"]
        return out

    if payload["Regime"] == "RISK OFF":
        return pd.DataFrame([
            {"Slot":"DEF1","Theme":"Cash / Short Bonds","MacroSector":"Defense","Ticker":"IBGS.MI","Score":70,"TargetWeight":4.0,"Regime":"RISK OFF","RiskBudget":"DEFENSIVE"},
            {"Slot":"DEF2","Theme":"Gold","MacroSector":"Gold","Ticker":"SGLD.MI","Score":65,"TargetWeight":2.0,"Regime":"RISK OFF","RiskBudget":"DEFENSIVE"},
            {"Slot":"DEF3","Theme":"Cash proxy","MacroSector":"Cash","Ticker":"XEON.MI","Score":60,"TargetWeight":1.0,"Regime":"RISK OFF","RiskBudget":"DEFENSIVE"},
        ])

    return pd.DataFrame([
        {"Slot":"CR1","Theme":"Cash / Short Bonds","MacroSector":"Crisis","Ticker":"IBGS.MI","Score":80,"TargetWeight":8.0,"Regime":"CRISIS","RiskBudget":"CRISIS"},
        {"Slot":"CR2","Theme":"Gold","MacroSector":"Crisis","Ticker":"SGLD.MI","Score":75,"TargetWeight":5.0,"Regime":"CRISIS","RiskBudget":"CRISIS"},
    ])

def regime_aware_pac_v90(real_df, pac_amount, vix_override=None):
    payload = regime_engine_v90(vix_override)
    budget = regime_risk_budget_v90(payload["Regime"])
    ...
    if payload["Regime"] in ["RISK ON", "NEUTRAL"]:
        if "pac_advisor_v82" in globals():
            return pac_advisor_v82(real_df, pac_amount, budget["PACMode"], vix_override)
        return pac_advisor(real_df, pac_amount, vix=vix_override)

    df = real_df.copy()
    defensive = df[df["ticker"].isin(["IBGS.MI", "SGLD.MI", "MTHP.MI", "CMOD.MI"])].copy()
    if defensive.empty:
        defensive = df.copy()

    defensive["gap_abs"] = defensive["deviation_pp"].clip(upper=0).abs()
    if defensive["gap_abs"].sum() <= 0:
        defensive["gap_abs"] = 1.0
    defensive["pac_amount"] = pac_amount * defensive["gap_abs"] / defensive["gap_abs"].sum()
    defensive["pac_mode"] = f"Regime-aware {payload['Regime']}"
    return defensive[["ticker","deviation_pp","pac_amount","pac_mode"]].sort_values("pac_amount", ascending=False)

def regime_trade_guard_v90(order_df, regime_payload):
    if order_df is None or order_df.empty:
        return order_df
    out = order_df.copy()
    regime = regime_payload.get("Regime", "NEUTRAL")
    if regime in ["RISK OFF", "CRISIS"]:
        out["regime_guard"] = np.where(
            out["ticker"].astype(str).isin(SATELLITE_TICKERS_V82 if "SATELLITE_TICKERS_V82" in globals() else ["RBOT.MI","DFNS.MI","SMH"]) & out["action"].astype(str).str.contains("BUY", na=False),
            "BLOCK_NEW_SATELLITE_BUY",
            "OK"
        )
    else:
        out["regime_guard"] = "OK"
...
# ======================================================
# V8.3 TAX OPTIMIZATION ENGINE
# ======================================================

TAX_LOTS_CSV = "tax_lots.csv"
TAX_EVENTS_CSV = "tax_events.csv"
DEFERRED_SELL_CSV = "deferred_sell_queue.csv"
        ...
        if columns:
            for c in columns:
                if c not in df.columns:
                    df[c] = 0 if c not in ["date","ticker","note","reason","status"] else ""
        return df
    except Exception:
...
def load_tax_events():
    cols = ["date","ticker","event_type","gross_value","realized_gain","tax_paid","note"]
    return load_csv_safe(TAX_EVENTS_CSV, cols)

def tax_budget_status(tax_budget_annual, tax_rate):
    events = load_tax_events()
    if events.empty:
        realized_gain = 0.0
        tax_paid = 0.0
    else:
        events["realized_gain"] = pd.to_numeric(events["realized_gain"], errors="coerce").fillna(0.0)
        events["tax_paid"] = pd.to_numeric(events["tax_paid"], errors="coerce").fillna(0.0)
        tax_paid = float(events["tax_paid"].sum())
        realized_gain = float(events["realized_gain"].sum())
    remaining_tax = max(0.0, float(tax_budget_annual) - tax_paid)
    remaining_gain_capacity = remaining_tax / (float(tax_rate)/100) if tax_rate and tax_rate > 0 else 0.0
    return {
        "realized_gain_ytd": realized_gain,
        "tax_paid_ytd": tax_paid,
        "tax_budget": float(tax_budget_annual),
        "remaining_tax_budget": remaining_tax,
        "remaining_gain_capacity": remaining_gain_capacity,
    }

def estimate_tax_on_sell_row(row, sell_amount, tax_rate):
    mv = float(row.get("market_value", 0) or 0)
    pnl = float(row.get("pnl", 0) or 0)
    if mv <= 0 or sell_amount <= 0:
        return 0.0, 0.0
    gain_ratio = max(0.0, pnl / mv)
    taxable_gain = sell_amount * gain_ratio
    tax_est = taxable_gain * (float(tax_rate)/100)
    return taxable_gain, tax_est

def tax_classification(tax_est, remaining_tax_budget):
    if tax_est <= 0:
        return "OK_NO_TAX"
    if tax_est <= remaining_tax_budget * 0.25:
        return "OK_LOW_TAX"
    if tax_est <= remaining_tax_budget:
        return "REVIEW_WITHIN_BUDGET"
    return "TAX_BLOCK"

def build_deferred_sell_queue(real_df, tax_rate=26.0, tax_budget_annual=2000.0, pac_amount=0.0):
    if real_df is None or real_df.empty:
        return pd.DataFrame()

    status = tax_budget_status(tax_budget_annual, tax_rate)
    remaining = status["remaining_tax_budget"]

    df = real_df.copy()
    action_col = "dynamic_action" if "dynamic_action" in df.columns else "action"
    sells = df[df[action_col].astype(str).isin(["SELL/TRIM", "SELL", "TRIM"])].copy()

    rows = []
    for _, r in sells.iterrows():
        sell_amount = abs(float(r.get("trade_value", 0) or 0))
        taxable_gain, tax_est = estimate_tax_on_sell_row(r, sell_amount, tax_rate)
        cls = tax_classification(tax_est, remaining)

        if pac_amount > 0 and cls in ["REVIEW_WITHIN_BUDGET", "TAX_BLOCK"]:
            recommendation = "DEFER_USE_PAC"
            reason = "Usa PAC/nuova liquidità per correggere sottopesi prima di vendere asset in gain."
        elif cls == "TAX_BLOCK":
            recommendation = "DEFER_JANUARY_OR_LOSS_OFFSET"
            reason = "Imposta stimata oltre il budget fiscale residuo."
        elif cls == "REVIEW_WITHIN_BUDGET":
            recommendation = "REVIEW_SELL"
            reason = "Vendita entro budget ma fiscalmente rilevante."
        else:
            recommendation = "SELL_ALLOWED"
            reason = "Impatto fiscale basso o nullo."

        rows.append({
            "ticker": r.get("ticker",""),
            "current_weight": r.get("current_weight",0),
            "target_weight": r.get("target_weight",0),
            "deviation_pp": r.get("deviation_pp",0),
            ...
            "reason": reason,
            "status": "OPEN" if recommendation.startswith("DEFER") else "ACTIONABLE",
        })

    out = pd.DataFrame(rows)
    if not out.empty:
        save_csv_safe(out, DEFERRED_SELL_CSV)
    return out

def tax_aware_order_plan(real_df, pac_amount=0.0, tax_rate=26.0, tax_budget_annual=2000.0):
    if real_df is None or real_df.empty:
        return pd.DataFrame()
    ...
    for _, r in df.iterrows():
        t = r.get("ticker","")
        action = str(r.get(action_col, r.get("action","HOLD")))
        trade_value = float(r.get("trade_value",0) or 0)

        if action in ["BUY/ADD", "BUY"]:
            rows.append({
                "priority": 1,
                "ticker": t,
                "action": "BUY_WITH_PAC",
                "amount": max(0.0, trade_value),
                "tax_est": 0.0,
                "tax_class": "NO_TAX",
                "reason": "Acquisto: nessun realizzo fiscale."
            })
        elif action in ["SELL/TRIM", "SELL", "TRIM"]:
            sell_amount = abs(trade_value)
            taxable_gain, tax_est = estimate_tax_on_sell_row(r, sell_amount, tax_rate)
            cls = tax_classification(tax_est, remaining)
            if pac_amount > 0 and cls in ["REVIEW_WITHIN_BUDGET", "TAX_BLOCK"]:
                act = "DEFER_SELL_USE_PAC"
                prio = 4
                reason = "Differisci vendita: prima usa PAC/nuova liquidità."
            elif cls == "TAX_BLOCK":
                act = "TAX_BLOCK"
                prio = 5
                reason = "Vendita bloccata dal budget fiscale."
            else:
                act = "SELL_ALLOWED"
                prio = 3
                reason = "Vendita fiscalmente accettabile."
            rows.append({
                "priority": prio,
                "ticker": t,
                "action": act,
            ...
            rows.append({
                ...
                "tax_est": 0.0,
                "tax_class": "NO_ACTION",
                "reason": "Nessuna azione."
            })

    out = pd.DataFrame(rows)
    if out.empty:
        return out
    return out.sort_values(["priority","tax_est"], ascending=[True, True])

def tax_alpha_score_adjustment(alpha_df, real_df, tax_rate=26.0):
    """
    Penalizza candidati che richiederebbero vendite fiscalmente onerose.
    Per gli acquisti non penalizza.
    """
    if alpha_df is None or alpha_df.empty:
        return alpha_df
    if real_df is None or real_df.empty:
        out = alpha_df.copy()
        out["TaxAdjustedAlpha"] = out["AlphaScore"]
        return out

    sell_queue = build_deferred_sell_queue(real_df, tax_rate=tax_rate, tax_budget_annual=10**9, pac_amount=0)
    high_tax_tickers = set()
    if sell_queue is not None and not sell_queue.empty:
        high_tax_tickers = set(sell_queue[sell_queue["estimated_tax"] > 0]["ticker"].astype(str).tolist())

    out = alpha_df.copy()
    out["TaxPenalty"] = out["Ticker"].astype(str).isin(high_tax_tickers).astype(int) * 10
    out["TaxAdjustedAlpha"] = (out["AlphaScore"] - out["TaxPenalty"]).clip(lower=0)
    return out.sort_values("TaxAdjustedAlpha", ascending=False)
...
def load_portfolio_primary(vix_override=None):
    """
    V8.3: carica prima portfolio_positions.csv.
...
# UI
# ══════════════════════════════════════════════════════════════════════════════
st.title("Portfolio Cockpit Alpha Pro v10.0")
st.caption("v10.0 — Regime & Crisis Engine · Dynamic Risk Budget · Crisis Protocol · Regime-aware PAC")
...
with st.sidebar:
    ...
    regime_mode_v90 = st.checkbox("Regime & Crisis Engine V9", value=True)
...
with tabs[0]:
    ...
    try:
        st.dataframe(regime_adjusted_satellite_plan_v90(vix_override=vix_override, mode=satellite_mode_v82 if "satellite_mode_v82" in globals() else "Static"), use_container_width=True)
    except Exception:
...
with tabs[1]:  # Institutional Dashboard
    st.subheader("🏛️ Institutional Dashboard V10.0")
    rt = st.session_state.get("real_table", pd.DataFrame())
    regime_v10 = regime_engine_v90(vix_override) if "regime_engine_v90" in globals() else {"Regime":"NEUTRAL","Score":50,"VIX":None}
    tax_status_v10 = tax_budget_status(tax_budget, tax_rate) if "tax_budget_status" in globals() else {"remaining_tax_budget":tax_budget,"tax_budget":tax_budget}
    health_v10 = portfolio_health_pro(rt, regime_v10, tax_status_v10)

    c1,c2,c3,c4,c5 = st.columns(5)
    c1.metric("Institutional Grade", health_v10["Grade"])
    c2.metric("Health Pro", f"{health_v10['TotalScore']:.0f}/100")
    c3.metric("Regime", regime_v10["Regime"])
    c4.metric("Risk Budget", regime_risk_budget_v90(regime_v10["Regime"])["RiskBudget"] if "regime_risk_budget_v90" in globals() else "NORMAL")
    c5.metric("Tax Budget Residuo", f"€{tax_status_v10.get('remaining_tax_budget',0):,.0f}")

    st.write("### Portfolio Health Pro")
    st.dataframe(pd.DataFrame([health_v10]), use_container_width=True)

    if rt is not None and not rt.empty:
        st.write("### Risk Budget Pro")
        st.dataframe(risk_budget_pro(rt, vix_override), use_container_width=True)

        st.write("### Alpha Score Pro")
        asp = alpha_score_pro(rt, vix_override)
        st.dataframe(asp, use_container_width=True)
        if not asp.empty:
            st.plotly_chart(px.bar(asp.head(10), x="Ticker", y="InstitutionalAlpha", color="AlphaGrade"), use_container_width=True)

        st.write("### Institutional Trade Blotter")
        blotter = trade_blotter_institutional(rt, vix_override, tax_rate, tax_budget, pac_amount)
        st.dataframe(blotter, use_container_width=True)
        if blotter is not None and not blotter.empty:
            st.download_button("Scarica trade_blotter.csv", data=blotter.to_csv(index=False).encode("utf-8"), file_name="trade_blotter.csv", mime="text/csv")
    else:
        st.info("Aggiorna prima My Portfolio.")

with tabs[2]:  # Investment Committee
    st.subheader("📝 Investment Committee Memo V10.0")
    rt = st.session_state.get("real_table", pd.DataFrame())
    memo = investment_committee_memo(rt, vix_override, tax_rate, tax_budget, pac_amount)
    md = memo_to_markdown(memo)
    st.markdown(md)
    note = st.text_area("Nota decisionale", value="")
    c1,c2 = st.columns(2)
    if c1.button("Salva nel Decision Log"):
        log = save_decision_log_entry(memo, note)
        st.success("Decisione salvata.")
        st.dataframe(log, use_container_width=True)
    c2.download_button("Scarica memo Markdown", data=md.encode("utf-8"), file_name="investment_committee_memo.md", mime="text/markdown")
    st.write("### Decision Log")
    log = load_decision_log()
    if log.empty: st.info("Nessuna decisione salvata.")
    else:
        st.dataframe(log, use_container_width=True)
        st.download_button("Scarica decision_log.csv", data=log.to_csv(index=False).encode("utf-8"), file_name="decision_log.csv", mime="text/csv")
...
with tabs[9]:  # Regime & Crisis V9
    st.subheader("🛡️ Regime & Crisis Engine V10.0")
    st.caption("Motore unico di regime: VIX, trend 200DMA, breadth, credito, macro, liquidità, tassi e dollaro.")

    regime_v90 = regime_engine_v90(vix_override)
    budget_v90 = regime_risk_budget_v90(regime_v90["Regime"])

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Regime", regime_v90["Regime"])
    c2.metric("Regime Score", f"{regime_v90['Score']:.0f}/100")
    c3.metric("VIX", "n/a" if regime_v90["VIX"] is None else f"{regime_v90['VIX']:.1f}")
    c4.metric("Risk Budget", budget_v90["RiskBudget"])
    c5.metric("PAC Mode", budget_v90["PACMode"])

    st.write("### Componenti del regime")
    st.dataframe(regime_v90["Components"], use_container_width=True)

    st.write("### Dynamic Risk Budget")
    st.dataframe(pd.DataFrame([budget_v90]), use_container_width=True)

    st.write("### Crisis Protocol")
    for item in crisis_protocol_v90(regime_v90):
        st.write("• " + item)

    st.write("### Satellite regime-adjusted")
    try:
        sat_v90 = regime_adjusted_satellite_plan_v90(vix_override=vix_override, mode=satellite_mode_v82 if "satellite_mode_v82" in globals() else "Static")
        st.dataframe(sat_v90, use_container_width=True)
    except Exception as e:
        st.warning(f"Satellite regime-adjusted non disponibile: {e}")

    st.write("### Regime-aware PAC")
    rt = st.session_state.get("real_table", pd.DataFrame())
    if rt is None or rt.empty:
        st.info("Aggiorna prima My Portfolio.")
    else:
        try:
            pac_v90 = regime_aware_pac_v90(rt, pac_amount, vix_override)
            st.dataframe(pac_v90, use_container_width=True)
        except Exception as e:
            st.warning(f"PAC regime-aware non disponibile: {e}")


with tabs[10]:  # Mercati
    out=[]
...
with tabs[12]:  # Relative Strength
    out=[]
...
with tabs[14]:  # Satellite Auto
    try:
        plan = regime_adjusted_satellite_plan_v90(vix_override=vix_override, mode=satellite_mode_v82)
    except Exception:
        plan = satellite_plan(vix_override=vix_override)
    st.dataframe(plan, use_container_width=True)

with tabs[15]:  # Backtest Pro
...
with tabs[20]:  # Tax Optimizer Pro
    st.subheader("🧾 Tax Optimizer Pro V10.0")
    st.caption("Motore fiscale operativo: budget annuale, vendite differite, PAC-first rebalancing e tax-aware order plan.")

    status = tax_budget_status(tax_budget, tax_rate)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Tax Budget", f"€{status['tax_budget']:,.0f}")
    c2.metric("Tax Paid YTD", f"€{status['tax_paid_ytd']:,.0f}")
    c3.metric("Budget residuo", f"€{status['remaining_tax_budget']:,.0f}")
    c4.metric("Gain capacity", f"€{status['remaining_gain_capacity']:,.0f}")

    st.write("### Eventi fiscali registrati")
    events = load_tax_events()
    if events.empty:
        st.info("Nessun evento fiscale registrato. Puoi aggiungerli sotto per tracciare il budget.")
    else:
        st.dataframe(events, use_container_width=True)

    with st.expander("Aggiungi evento fiscale manuale"):
        with st.form("tax_event_form", clear_on_submit=True):
            ev_date = st.text_input("Data", value=datetime.utcnow().strftime("%Y-%m-%d"))
            ev_ticker = st.text_input("Ticker", value="")
            ev_type = st.selectbox("Tipo evento", ["SELL", "DIVIDEND", "TAX_PAYMENT", "OTHER"])
            ev_gross = st.number_input("Valore lordo", 0.0, 1000000.0, 0.0, 10.0)
            ev_gain = st.number_input("Plusvalenza realizzata", -1000000.0, 1000000.0, 0.0, 10.0)
            ev_tax = st.number_input("Imposta pagata/stimata", 0.0, 1000000.0, 0.0, 10.0)
            ev_note = st.text_input("Note", value="")
            if st.form_submit_button("Salva evento fiscale"):
                new_ev = pd.DataFrame([{
                    "date": ev_date, "ticker": ev_ticker, "event_type": ev_type,
                    "gross_value": ev_gross, "realized_gain": ev_gain,
                    "tax_paid": ev_tax, "note": ev_note
                }])
                updated = pd.concat([events, new_ev], ignore_index=True)
                save_csv_safe(updated, TAX_EVENTS_CSV)
                st.success("Evento fiscale salvato. Ricarica la pagina per aggiornare i metric.")
                st.rerun()
    ...
    if "real_table" not in st.session_state:
        ...
        st.write("### Tax-aware Order Plan")
        tax_plan = tax_aware_order_plan(rt, pac_amount, tax_rate, tax_budget)
        if tax_plan.empty:
            st.success("Nessuna operazione fiscale da valutare.")
        else:
            st.dataframe(tax_plan, use_container_width=True)

        st.write("### Deferred Sell Queue")
        deferred = build_deferred_sell_queue(rt, tax_rate, tax_budget, pac_amount)
        if deferred.empty:
            st.success("Nessuna vendita da differire.")
        else:
            st.dataframe(deferred, use_container_width=True)
            st.download_button(
                "Scarica deferred_sell_queue.csv",
                data=deferred.to_csv(index=False).encode("utf-8"),
                file_name="deferred_sell_queue.csv",
                mime="text/csv"
            )

        st.write("### Interpretazione operativa")
        st.info(
            "Gerarchia V10.0: 1) usa PAC/nuova liquidità; 2) compra sottopesi; "
            "3) vendi solo se entro budget fiscale; 4) differisci vendite con fiscal drag elevato."
        )


with tabs[21]:  # Tax Engine
...
with tabs[24]:  # Scenario Simulator
...
with tabs[26]:  # Alerts
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v10.0: test alert.")
...
with tabs[27]:
    ...
    if "real_table" in st.session_state:
        ...
        st.caption("V8.3 candidate: score preliminare per allocazione alpha.")
