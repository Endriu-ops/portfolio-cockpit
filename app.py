...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v2.2", layout="wide")
...
def satellite_plan():
    ...
    for i,(_,r) in enumerate(elig.iterrows()):
        out.append({"Slot":f"SAT{i+1}","Theme":r["Theme"],"Ticker":r["Ticker"],"Score":r["Score"],"TargetWeight":[5,5,3][i]})
    for j in range(len(out),3):
        out.append({"Slot":f"SAT{j+1}","Theme":"Cash/Bond breve","Ticker":"XEON.MI","Score":None,"TargetWeight":[5,5,3][j]})
...
def price_matrix(weights, period):
    ...
    pm=pm.apply(pd.to_numeric, errors="coerce").dropna(how="all").ffill().dropna()
    return pm
...
def stats(eq):
    ...
    r=eq.pct_change().dropna(); years=len(r)/12; cagr=(eq.iloc[-1]/eq.iloc[0])**(1/years)-1; vv=r.std()*math.sqrt(12); draw=eq/eq.cummax()-1
    return {"CAGR":cagr,"Volatility":vv,"Max Drawdown":draw.min(),"Sharpe approx":cagr/vv if vv>0 else np.nan,"Final Multiple":eq.iloc[-1]/eq.iloc[0]}
...
st.title("Portfolio Cockpit Alpha Pro v2.2")
st.caption("Versione senza SQLite: portafoglio reale + backtest robusto + satellite automatico.")
with st.sidebar:
...
with tabs[1]:
    ...
    if st.session_state["snapshots"]:
        snap=pd.DataFrame(st.session_state["snapshots"]).set_index("time")
        st.line_chart(snap["total"])
...
with tabs[10]:
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v2.2: test alert.")
