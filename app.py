...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v7.0", layout="wide")
...
st.title("Portfolio Cockpit Alpha Pro v7.0")
st.caption("V7.0: Professional Backtest, VIX Adaptive Engine, Attribution, Fiscal Optimizer, Crisis Dashboard.")
...
with tabs[12]:
    st.subheader("Professional Backtest")
    st.caption("Mostra periodo reale, anni effettivi, CAGR verificato, excess return, information ratio e hit ratio.")
    ...
    if st.button("Esegui Professional Backtest"):
        ...
        if not series:
            ...
            st.line_chart(df_bt)
            st.write("### Diagnostica periodo")
            p1,p2,p3=st.columns(3)
            p1.metric("Start", str(df_bt.index[0].date()))
            p2.metric("End", str(df_bt.index[-1].date()))
            p3.metric("Years", f"{((df_bt.index[-1]-df_bt.index[0]).days/365.25):.2f}")
            ...
            if ((df_bt.index[-1]-df_bt.index[0]).days/365.25) < 9 and btperiod == "10y":
                st.warning("Il backtest richiesto è 10y, ma i dati disponibili coprono meno di 9 anni. Il CAGR va interpretato come annualizzazione del periodo effettivo.")
...
with tabs[21]:
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v7.0: test alert.")
