...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v5.1", layout="wide")
...
def satellite_plan(max_per_macro=1, vix_override=None):
    ...
    if scs["DefensivePressure"]>=80 or (v is not None and v>=60):
        return pd.DataFrame([{"Slot":"SAT1","Theme":"Cash/Bond breve","MacroSector":"Cash","Ticker":"XEON.MI","Score":None,"TargetWeight":5.0},{"Slot":"SAT2","Theme":"Cash/Bond breve","MacroSector":"Cash","Ticker":"XEON.MI","Score":None,"TargetWeight":5.0},{"Slot":"SAT3","Theme":"Gold / safe haven","MacroSector":"Gold","Ticker":"SGLD.MI","Score":None,"TargetWeight":3.0}])
...
def real_table(df, targets, band):
    ...
    total=x["market_value"].sum(); x["current_weight"]=0 if total<=0 else x["market_value"]/total*100
    x["target_weight"]=x["ticker"].map(targets).fillna(0); x["deviation_pp"]=x["current_weight"]-x["target_weight"]
...
def default_portfolio_rows(vix_override=None):
    ...
    rows = [{"ticker": t, "shares": 0.0, "avg_price": 0.0, "broker": "", "note": ""} for t in CORE]
    for _, r in plan.iterrows():
        if r["Ticker"] not in [x["ticker"] for x in rows]:
            rows.append({"ticker": r["Ticker"], "shares": 0.0, "avg_price": 0.0, "broker": "", "note": ""})
    return pd.DataFrame(rows)
def load_portfolio_csv(vix_override=None):
    try:
        df=pd.read_csv(PORTFOLIO_CSV)
        for col in ["ticker","shares","avg_price","broker","note"]:
        ...
        return df[["ticker","shares","avg_price","broker","note"]]
    except Exception:
...
def save_portfolio_csv(df):
    ...
    out[["ticker","shares","avg_price","broker","note"]].to_csv(PORTFOLIO_CSV,index=False)
...
def portfolio_from_transactions(tx):
    ...
    for ticker,g in tx.groupby("ticker"):
        ...
        rows.append({"ticker":ticker,"shares":qty_total,"avg_price":avg_price,"broker":broker,"note":"auto PMC da transazioni"})
    return pd.DataFrame(rows)
def merge_template_with_saved(saved, vix_override=None):
    ...
    out=saved.copy(); existing=set(out["ticker"].astype(str))
    missing=template[~template["ticker"].astype(str).isin(existing)]
    return pd.concat([out,missing],ignore_index=True)[["ticker","shares","avg_price","broker","note"]]
...
st.title("Portfolio Cockpit Alpha Pro v5.1")
st.caption("Decision Center + salvataggio portafoglio CSV + storico transazioni e PMC automatico.")
...
tabs=st.tabs(["Decision Center","Allocation","My Portfolio","Transactions","Scores","Market","FRED","Relative Strength","Tactical Ranking","Satellite Auto","Backtest","Tax Engine","Scenario Simulator","Paper Trading","Alerts"])
...
with tabs[2]:
    st.subheader("My Portfolio — persistente")
    csave1,csave2,csave3,csave4=st.columns(4)
    if csave1.button("Carica da CSV"):
        st.session_state["portfolio_df"]=merge_template_with_saved(load_portfolio_csv(vix_override),vix_override); st.rerun()
    if csave2.button("Salva portafoglio"):
        save_portfolio_csv(st.session_state["portfolio_df"]); st.success("Salvato.")
    if csave3.button("Usa PMC da transazioni"):
        st.session_state["portfolio_df"]=merge_template_with_saved(portfolio_from_transactions(st.session_state["transactions_df"]),vix_override); save_portfolio_csv(st.session_state["portfolio_df"]); st.rerun()
    if csave4.button("Reset template"):
        st.session_state["portfolio_df"]=default_portfolio_rows(vix_override); st.warning("Template ripristinato.")
    pos=st.data_editor(st.session_state["portfolio_df"],use_container_width=True,num_rows="dynamic",key="portfolio_editor")
    st.session_state["portfolio_df"]=pos
    if st.button("Aggiorna portafoglio reale"):
        save_portfolio_csv(pos)
        plan=satellite_plan(vix_override=vix_override); targets={k:v[1] for k,v in CORE.items()}
        for _,r in plan.iterrows(): targets[r["Ticker"]]=targets.get(r["Ticker"],0)+float(r["TargetWeight"])
        tab=real_table(pos,targets,band); st.session_state["real_table"]=tab; st.session_state["snapshots"].append({"time":datetime.utcnow(),"total":tab["market_value"].sum()}); st.rerun()
...
with tabs[3]:
    ...
    tx=st.data_editor(st.session_state["transactions_df"],use_container_width=True,num_rows="dynamic",key="transactions_editor",column_config={"side": st.column_config.SelectboxColumn("side", options=["BUY","SELL"])})
    st.session_state["transactions_df"]=tx
    t1,t2,t3=st.columns(3)
    if t1.button("Salva transazioni"):
        save_transactions_csv(tx); st.success("Transazioni salvate.")
    if t2.button("Carica transazioni"):
        st.session_state["transactions_df"]=load_transactions_csv(); st.rerun()
    if t3.button("Calcola portafoglio da transazioni"):
        st.dataframe(portfolio_from_transactions(tx),use_container_width=True)
...
with tabs[14]:
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v5.1: test alert.")
