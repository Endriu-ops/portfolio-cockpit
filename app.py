...
st.set_page_config(page_title="Portfolio Cockpit Alpha Pro v7.3", layout="wide")
...
PRELOADED_PORTFOLIO = [{'ticker': 'IBGS.MI', 'shares': 98.0, 'avg_price': 139.9, 'manual_price': 0, 'broker': '', 'note': 'Core Bond Breve'}, {'ticker': 'MTHP.MI', 'shares': 193.0, 'avg_price': 73.1012, 'manual_price': 0, 'broker': '', 'note': 'Core Bond Lungo'}, {'ticker': 'SGLD.MI', 'shares': 70.974, 'avg_price': 208.48, 'manual_price': 0, 'broker': '', 'note': 'Gold'}, {'ticker': 'SXR8', 'shares': 10.419, 'avg_price': 588.85, 'manual_price': 0, 'broker': '', 'note': 'SP500 Legacy'}, {'ticker': 'ZPRV', 'shares': 407.0, 'avg_price': 66.241, 'manual_price': 0, 'broker': '', 'note': 'Small Cap Value'}, {'ticker': 'BTC', 'shares': 0.126, 'avg_price': 70381.31, 'manual_price': 0, 'broker': '', 'note': 'BTC Spot'}, {'ticker': 'CMOD.MI', 'shares': 460.0, 'avg_price': 27.195, 'manual_price': 0, 'broker': '', 'note': 'Commodities'}, {'ticker': 'GDX.MI', 'shares': 110.0, 'avg_price': 85.01, 'manual_price': 0, 'broker': '', 'note': 'Gold Miners'}, {'ticker': 'VWCE', 'shares': 4.0, 'avg_price': 161.97, 'manual_price': 0, 'broker': '', 'note': 'Global Equity'}, {'ticker': 'VUAA', 'shares': 203.0, 'avg_price': 108.4737, 'manual_price': 0, 'broker': '', 'note': 'SP500 Core'}, {'ticker': 'DFNS.MI', 'shares': 146.0, 'avg_price': 53.46, 'manual_price': 0, 'broker': '', 'note': 'Defense'}, {'ticker': 'RBOT.MI', 'shares': 428.0, 'avg_price': 18.26, 'manual_price': 0, 'broker': '', 'note': 'AI Robotics'}, {'ticker': 'SMH', 'shares': 46.0, 'avg_price': 101.18, 'manual_price': 0, 'broker': '', 'note': 'Tactical'}]
...
def clean_portfolio_df(df):
    ...
    df["ticker"] = df["ticker"].replace({"CBTC.MI":"BTC", "BTC-USD":"BTC", "XZPRV":"ZPRV", "DFEN.MI":"DFNS.MI"})
...
ALPHA_STATIC = {"IBGS.MI":8, "MTHP.MI":4, "SGLD.MI":15, "SXR8":22, "ZPRV":19, "BTC":5, "CMOD.MI":8, "GDX.MI":6, "DFNS.MI":5, "RBOT.MI":5, "SMH":3}
...
def attribution_engine(real_df):
    ...
    mapping = {
        ...
        "CMOD.MI":"Commodities", "RBOT.MI":"AI", "DFNS.MI":"Defense",
        "SMH":"Tactical"
    }
    df = real_df.copy()
...
def default_portfolio_rows(vix_override=None):
    ...
    base["ticker"] = base["ticker"].astype(str).replace({"CBTC.MI":"BTC", "BTC-USD":"BTC", "DFEN.MI":"DFNS.MI"})
...
st.title("Portfolio Cockpit Alpha Pro v7.3")
st.caption("V7.3 hotfix: PAC advisor scalar abs fix, DFNS/RBOT permanent, Extended 10Y Backtest.")
...
with tabs[18]:
    ...
    if bad.empty:
        st.success("Tutti i ticker hanno prezzo YAHOO o MANUAL valido.")
    else:
        st.warning("Alcuni ticker usano fallback PMC o non hanno prezzo. Inserisci manual_price o correggi il ticker.")
...
with tabs[21]:
    msg=st.text_area("Messaggio","Portfolio Cockpit Alpha Pro v7.3: test alert.")
